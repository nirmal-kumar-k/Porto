 ...
 *   z = c*sin(2t)
 *
 * Two identical seams are rotated 90° apart on the sphere surface.
 */
export function createSeamCurve(
  radius: number,
  segments = 256,
  rotationY = 0,
  ...
  const b = 0.3
  const c = 0.99
  const rotMatrix = rotationY !== 0 ? new THREE.Matrix4().makeRotationY(rotationY) : null
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    if (rotMatrix) v.applyMatrix4(rotMatrix)
    points.push(v)
  }
